...
func TestGranularPermissions(t *testing.T) {
	t.Parallel()
	admin := CreateAdminUser(t, "granular")
	ctx := context.Background()

	// Create an unprivileged user in the same tenant
	user := CreateUserInTenant(t, admin, "granular-user")

	t.Run("role:read allows list and get but not create", func(t *testing.T) {
		roleReadPerm := GetPermissionByName(t, admin.Client, "role:read")
		...
		AssertAPIError(t, err, http.StatusForbidden, "role:read should not allow create")
	})

	t.Run("deny overrides allow from role", func(t *testing.T) {
		roleReadPerm := GetPermissionByName(t, admin.Client, "role:read")
		roleCreatePerm := GetPermissionByName(t, admin.Client, "role:create")
		...
		_, err = user.Client.ListRoles(ctx)
		AssertAPIError(t, err, http.StatusForbidden, "deny should override allow")
	})
}
