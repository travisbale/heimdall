...
// Use this for operations that don't require tenant scoping (e.g., creating tenants,
// verification tokens).
...
//	err := db.WithTenantContext(ctx, tenantID, func(q *sqlc.Queries) error {
//	    dbUser, err := q.GetUser(ctx, userID)
//	    if err != nil {
//	        return err
//	    }
//	    user, err = convertToDomain(dbUser)
//	    return err
//	})
func (d *DB) WithTenantContext(ctx context.Context, tenantID uuid.UUID, fn func(*sqlc.Queries) error) error {
	// Begin transaction
	tx, err := d.pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("failed to begin transaction: %w", err)
	...
	// Rollback is safe to call even if the transaction is later committed
	// If commit succeeds, rollback will be a no-op; if commit fails, rollback cleans up
	defer func() {
		if err2 := tx.Rollback(ctx); err2 != nil {
			slog.Error("failed to rollback transaction", "error", err2)
		}
