...
const deleteOIDCLinkByUserAndProvider = `-- name: DeleteOIDCLinkByUserAndProvider :exec
DELETE FROM oidc_links
WHERE user_id = $1 AND oidc_provider_id = $2
`

type DeleteOIDCLinkByUserAndProviderParams struct {
	UserID         uuid.UUID `json:"user_id"`
	OidcProviderID uuid.UUID `json:"oidc_provider_id"`
}

func (q *Queries) DeleteOIDCLinkByUserAndProvider(ctx context.Context, arg DeleteOIDCLinkByUserAndProviderParams) error {
	_, err := q.db.Exec(ctx, deleteOIDCLinkByUserAndProvider, arg.UserID, arg.OidcProviderID)
	return err
}
