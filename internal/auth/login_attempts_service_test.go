...
func TestRecordFailedLogin_FifthAttempt_5MinuteLockout(t *testing.T) {
	...
	before := time.Now()
	err := service.RecordFailedLogin(ctx, "user@example.com", nil, nil, nil)
	after := time.Now()
...
func TestRecordFailedLogin_WithLastLoginAt(t *testing.T) {
	...
	err := service.RecordFailedLogin(ctx, "user@example.com", nil, nil, &lastLogin)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestRecordSuccessfulLogin_DeletesOldAttempts(t *testing.T) {
	...
	err := service.RecordSuccessfulLogin(ctx, "user@example.com", nil, nil)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestLoginAttemptsFlow_SuccessfulLoginResetsLockout(t *testing.T) {
	...
	err = service.RecordSuccessfulLogin(ctx, email, nil, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
