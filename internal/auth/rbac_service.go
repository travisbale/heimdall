...
func (s *RBACService) CreateRole(ctx context.Context, name, description string) (*Role, error) {
	...
	s.logger.Info("role created", "role_id", role.ID, "name", name)
	return role, nil
}
...
func (s *RBACService) UpdateRole(ctx context.Context, roleID uuid.UUID, name, description string) (*Role, error) {
	...
	s.logger.Info("role updated", "role_id", roleID)
	return role, nil
}
...
func (s *RBACService) SetRolePermissions(ctx context.Context, roleID uuid.UUID, permissionIDs []uuid.UUID) error {
	...
	s.logger.Info("role permissions updated", "role_id", roleID, "permission_count", len(permissionIDs))
	return nil
}
...
func (s *RBACService) SetUserRoles(ctx context.Context, userID uuid.UUID, roleIDs []uuid.UUID) error {
	...
	s.logger.Info("user roles updated", "user_id", userID, "role_count", len(roleIDs))
	return nil
}
...
func (s *RBACService) SetDirectPermissions(ctx context.Context, userID uuid.UUID, permissions []DirectPermission) error {
	...
	s.logger.Info("user permissions updated", "user_id", userID, "permission_count", len(permissions))
	return nil
}
...
func (s *RBACService) SetupSystemAdminRole(ctx context.Context, userID uuid.UUID) error {
	...
	permissions, err := s.permissionsDB.ListPermissions(ctx)
	if err != nil {
	...
	err = s.rolePermissionsDB.SetRolePermissions(ctx, role.ID, permissionIDs)
	if err != nil {
		return fmt.Errorf("failed to assign permissions to System Admin role: %w", err)
	}

	err = s.userRolesDB.SetUserRoles(ctx, userID, []uuid.UUID{role.ID})
	if err != nil {
		return fmt.Errorf("failed to assign System Admin role to user: %w", err)
	}

	s.logger.Info("System Admin role created and assigned", "user_id", userID, "role_id", role.ID, "permission_count", len(permissionIDs))
	return nil
}
